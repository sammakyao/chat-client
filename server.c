...
/*
The simple struct to hold the chatDataVars and the new client's socket fd.
...
// Here it spawns the new client handler thread and message consumer thread
void startChat(int socketFd)
{
    chatDataVars data;
    data.numClients = 0;
    ...
    //Starts thread to handle new client connections
    ...
    //Starts thread to handle messages received
...
//Below Removes the socket from the list of active client sockets and closes it
void removeClient(chatDataVars *data, int clientSocketFd)
{
    ...
    for(int i = 0; i < MAX_BUFFER; i++)
    {
        ...
        {
            write(data->clientSockets[i], "6", MAX_BUFFER - 1);
...
void *newClientHandler(void *data) {
    ...
    while (1) {
        ...
        if ((clientSocketFd > 0) && (strcmp(&msgBuffer[0], "0") == 0)) {
            numBytesRead = read(clientSocketFd, msgBuffer, MAX_BUFFER - 1);
            fprintf(stderr, "Server accepted new client. Client: %s\n", msgBuffer);
            ...
            if (chatData->numClients < MAX_BUFFER) {
                ...
                if ((pthread_create(&clientThread, NULL, (void *) &clientHandler, (void *) &chv)) == 0) {
                    chatData->numClients++;
                    fprintf(stderr, "Client has joined chat. Client: %s\n", msgBuffer);
                } else
                    close(clientSocketFd);
            }
...
void *clientHandler(void *chv)
{
    ...
    while(1)
    {
        ...
        if(strcmp(&msgBuffer[0], "5") == 0)
        {
            ...
            fprintf(stderr, "Pushing message to queue: %s", msgBuffer);
...
void *messageHandler(void *data)
{
    ...
    while(1)
    {
        ...
        fprintf(stderr, "Broadcasting message: %s", msg);
